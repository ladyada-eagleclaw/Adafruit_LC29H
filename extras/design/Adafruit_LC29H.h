...
// Shared parser dependency from Adafruit GPS; see README.md for the revision.
...
class Adafruit_LC29H : public Adafruit_GNSS {
 public:
  ...
  /** @brief Propose a receiver using caller-owned storage.
   *  @param firstBuffer First writable receive buffer.
   *  @param secondBuffer Second, non-overlapping writable receive buffer.
   *  @param capacity Bytes per buffer, including space for NUL. */
   ...
   *  @return Owned reply result, including its decoding status. */
  lc29h_pair_ack_t lastPairAck() const;
  /** @brief Decode the latest complete line as a firmware reply.
   *  @return Reply result with borrowed text; check status before use. */
  ...
  /** @brief Decode a validated PAIR001 sentence view.
   *  @param sentence Shared-validator result with unchanged source storage.
   *  @return Owned acknowledgment, or a decoding failure with cleared values.
   */
  ...
  /** @brief Decode a validated PQTMVERNO sentence view.
   *  @param sentence Shared-validator result with unchanged source storage.
   *  @return Borrowed firmware text, receiver error, or decoding failure. */
